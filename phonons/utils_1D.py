...
def rebuild_system_def(Nx,Ny,dir,defo,freeze_indices=None,dV=0,relax=False,constrain=False,Dz=0,constrain_bottom_indices=None,constrain_top_indices=None):
    #layer=np.loadtxt(f"layer_{dir}.txt")
    layer=np.zeros(8*Nx*Ny)
    layer+=1
    layer[Nx*Ny*4:]+=layer[Nx*Ny*4:] # second half is layer 2
    ...
    if relax:
        ...
        if freeze_indices is not None:
            ...
            input_lines[25]="fix 1 freeze setforce 0.0 NULL NULL\n"
            if constrain:
                input_lines[27]="group mv_bottom id "
                input_lines[28]="group mv_top id "
                input_lines[29]="group press id "
                for index in constrain_bottom_indices:
                    input_lines[27]+=f"{index} "
                    input_lines[29]+=f"{index} "
                for index in constrain_top_indices:
                    input_lines[28]+=f"{index} "
                    input_lines[29]+=f"{index} "
                input_lines[27]+="\n"
                input_lines[28]+="\n"
                input_lines[29]+="\n"
                input_lines[30]=f"displace_atoms mv_bottom move 0.0 0.0 {-(Dz-c)/2} units box\n"
                input_lines[31]=f"displace_atoms mv_top move 0.0 0.0 {(Dz-c)/2} units box\n"
                input_lines[32]=f"fix 2 press setforce NULL NULL 0.0\n"
            else:
                input_lines[30]="\n"#f"displace_atoms mv_bottom move 0.0 0.0 -0.5 units box\n"
                input_lines[31]="\n"#f"displace_atoms mv_top move 0.0 0.0 0.5 units box\n"
                input_lines[32]="\n"#f"fix 2 press setforce NULL NULL 0.0\n"
...
def file2atoms(filename,dV=0):
    system=read(filename,format="lammps-data")
    natoms=len(system)
    layer=np.zeros(natoms)
    layer+=1
    layer[natoms//2:]+=layer[natoms//2:] # second half is layer 2
    c=3.3306
    system.set_array("mol-id", layer, dtype=int)
    born, charges_lammps,charges=born_charges(system)
