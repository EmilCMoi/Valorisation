...
def build_1D_deformation(Nx,Ny,dir,def_f,params,fileName):
    ...
    for i in range(Nx):
        for j in range(Ny):
            ...
            charge[4*(i+j*Nx)]=0.42
            charge[4*(i+j*Nx)+1]=-0.42
            charge[4*(i+j*Nx)+2]=0.42
            charge[4*(i+j*Nx)+3]=-0.42
    ...
    datdat.append("Atoms # full\n\n")
    for i in range(len(pos_b)):
        datdat.append(f"{i+1} 1 {i%2+1} {charge[i]} {pos_b[i,0]} {pos_b[i,1]} 50.0\n")
        datdat.append(f"{len(pos_b)+i+1} 2 {i%2+1} {charge[i]} {pos_t[i,0]} {pos_t[i,1]} {50+c[i]}\n")
    dat = open(fileName,'w')
    dat.writelines(datdat)
    dat.close()
    #plt.show()
