...
def neb_deformed(Ns,dir,replicas,Nt=1,df=0.01,dV=0):
    # Ns is the number of unit cells in the wall
    # la is 'y' for zigzag, 'x' for armchair
    ...
    images=[system0]
    images+=[system0.copy() for _ in range(replicas-2)]
    images+=[system1]
    neb=NEB(images)
    neb.interpolate()
    for i in range(len(images)):
        #images[i].set_calculator(LAMMPS())
        images[i].set_array("mol-id", system0.get_array("mol-id"))
        born, charges_lammps, charges=born_charges(images[i])
        images[i].set_initial_charges(charges_lammps)
        images[i].set_array("born", born)
        images[i].set_array("charges_2", charges)
        images[i].set_array("charges_model", charges)
        voltage=[dV/2,-dV/2]
        voltages=np.zeros(len(images[i]))
        voltages[images[i].get_array("mol-id")==1] = voltage[0]
        voltages[images[i].get_array("mol-id")==2] = voltage[1]
        images[i].set_array("voltage", voltages)
        images[i].calc = LAMMPS()

    optimizer = LBFGS(neb, trajectory='A2B.traj')
    optimizer.run(fmax=df)
    traj=Trajectory('A2B.traj', 'r')
    energies=np.zeros(replicas)
    dipoles=np.zeros(replicas)
    for i in range(replicas):
        atoms=traj[-1-i]
        atoms.set_array("mol-id", system0.get_array("mol-id"))
        born, charges_lammps, charges=born_charges(atoms)
        atoms.set_initial_charges(charges_lammps)
        atoms.set_array("born", born)
        atoms.set_array("charges_2", charges)
...
neb_deformed(Ns=200,dir='0',replicas=20,df=0.01,dV=3.0)
    #system0.calc = LAMMPS()
    #system1.calc = LAMMPS()