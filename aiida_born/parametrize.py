...
def born_parametrized(atoms,C,beta,C2,beta2):#,dd,eps):
    """
    Calculate registry-dependent BECs and charges for a given structure.
    """
    #z0=0
    ...
    # Setting up parameters
    z0=3
    born_params={"B":{"N":{"C":C,"beta":beta},'z0':z0},"N":{"B":{},"N":{},'z0':-z0}}
    born_paramss=copy.deepcopy(born_params)
    born_paramss["N"]["B"]["beta"]=beta2#copy.deepcopy(born_params["B"]["N"]["beta"])
    born_paramss["N"]["B"]["C"]=C2#-copy.deepcopy(born_params["B"]["N"]["C"])
    pos=atoms.get_positions()
    cell=atoms.get_cell()
    rcut=10
    # Setting up neighbor list
    ...
    for i in range(na):
        charges[i]=born_paramss[atoms.symbols[i]]['z0']  # Initializing charges with z0
        if atoms.get_array("mol-id")[i] == 1:
            indices, offsets = nlborn.get_neighbors(i)
            for j, offset in zip(indices, offsets):
                if atoms.get_array("mol-id")[j]!=1 and atoms.symbols[i]!= atoms.symbols[j] and atoms.symbols[i]!="C" and atoms.symbols[j]!="C":
...
def fit(dummy, d0,a,C2,beta2):
    ...
    for struct in structs:
        ...
        born = np.ravel(born_parametrized(struct, d0,a,C2,beta2))
...
def plot_guess(results):
    ...
    for i, struct in enumerate(structs):
        struct.wrap()
        #print(struct.pbc)
        born =born_parametrized(struct,*results[0])
        bornss_calc[i] = born
...
results=curve_fit(fit, dummyy, borns_good,maxfev=10000)#,p0=[0.64875374, 1.58706567, 1.88588162, -1.60477028, 1.82270341, 0.45473178])
print(fit(dummyy, *results[0]))

print("Fitted parameters: ", results[0])
print(results)
...
plt.plot(fit(dummyy, *results[0]),borns_good, 'o')
