...
def launch_pw_workchain(structure):
    ...
    builder.pw.metadata.options.max_wallclock_seconds = 4 * 60  # 10 minutes
...
def launch_ph_workchain(pwWorkchain):
    ...
    builder.ph.metadata.options.max_wallclock_seconds = 10 * 60  # 15 minutes
...
def create_structure(r): # r is height of the cell in z direction
    a = 2.511
    v1 = a*np.array([1,0,0])
    ...
    labels = ["B", "N"]
    atoms = StructureData()
    struct=Atoms(symbols=labels, positions=positions,cell=[v1, v2, [0, 0, r]])
...
group = orm.load_group(31)
N=21
rs=np.linspace(15,35,N)
for i in range(N):
            r = rs[i] #np.random.normal(0, 0.2, 3)
...
qb=orm.QueryBuilder()
qb.append(orm.Group, filters={'id': 31}, tag='group')
