...
def launch_pw_workchain(structure):
    ...
    builder.pw.metadata.options.max_wallclock_seconds = 3 * 60  # 3 minutes
...
def launch_ph_workchain(pwWorkchain):
    ...
    builder.ph.metadata.options.max_wallclock_seconds = 5 * 60  # 5 minutes
...
def create_structure(r): # r is height difference between the two atoms
    a = 2.511
    v1 = a*np.array([1,0,0])
    ...
    positions[1] = np.array([0, 0, 0]) + v1 / 3 - v2 / 3+np.array([0,0,r])
...
group = orm.load_group(33)
N=21
# 2.1, 2.9, N=21
# 1.5, 2.1, N=15
rs=np.linspace(-0.3,0.3,N,endpoint=False)
print(rs)
...
'''
qb=orm.QueryBuilder()
qb.append(orm.Group, filters={'id': 33}, tag='group')
