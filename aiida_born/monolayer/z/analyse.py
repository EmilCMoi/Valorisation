...
def exp_quadratic(x, a, b, c,e):
    return np.exp(e * x)*(a * x**2 + b * x + c)
...
qb_ph.append(orm.Group, filters={'id': 33}, tag='group')
...
for structure, ph in qb_ph.all():
    ...
    structs.append(struct)
    a_s[count-1]=struct.positions[1,2]-struct.positions[0,2]#np.linalg.norm(struct.cell[0])
...
plt.figure()
plt.plot(a_s, borns[:,0,0,0], 'rx')
plt.grid(True)
plt.figure()
plt.plot(a_s, borns[:,0,1,1], 'rx')
plt.grid(True)
plt.figure()
plt.plot(a_s, borns[:,0,2,2], 'rx')
plt.grid(True)
plt.show()
...
plt.show()
'''
np.savez("borns_size.npz", borns=borns,sizes=a_s)
#print("after")
plt.show()
#print(rs)
#print(np.unique(rs))
