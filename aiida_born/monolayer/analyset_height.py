...
qb_ph.append(orm.Group, filters={'id': 31}, tag='group')
...
heights=np.zeros((len(qb_ph.all())))
...
for structure, ph in qb_ph.all():
    ...
    structs.append(struct)
    heights[count-1]=struct.cell[2][2]
...
plt.figure()
plt.plot(heights, borns[:,0,0,0])
plt.figure()
plt.plot(heights, borns[:,0,1,1])
plt.figure()
plt.plot(heights, borns[:,0,2,2])
plt.show()
'''
print(rs)
...
plt.show()
'''
#np.savez("borns_height.npz", borns=borns,structs=structs)
#print("after")
