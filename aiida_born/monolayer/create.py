...
def launch_ph_workchain(pwWorkchain):
    ...
    builder.ph.metadata.options.max_wallclock_seconds = 15 * 60  # 15 minutes
...
def create_structure(r): # r is some gaussian noise
    a = 2.511
    v1 = a*np.array([1,0,0])
...
group = orm.load_group(30)
N=20
for i in range(N):
            r = np.random.normal(0, 0.2, 3)
...
qb=orm.QueryBuilder()
qb.append(orm.Group, filters={'id': 30}, tag='group')
